...
| :---: | :--- | :--- | :--- | :--- | :---: | :--- |
| **1** | **High-performance React Web Apps** | JS runtime, Event Loop, Virtual DOM / Fiber, Render vs Commit, Reconciliation, memoization (`useMemo`, `useCallback`, `React.memo`), profiler | `02_FOUNDATIONS/03_REACT` & `08_PERFORMANCE` | Monorepos in `axis_clean`, `science-of-our-world`; verified Q3.1 (Fiber/reconciliation), Q3.2 (Render vs Commit), Q3.3 (Keys & 4-layer identity agreement) | 🛡️ INTERVIEW READY (Q3.1, Q3.2, Q3.3 Verified) | Explain component identity, reconciliation work vs browser work, and async safety; do not claim 7 commercial years. |
...
| **7** | **REST / Web APIs & Microservices** | HTTP methods, status codes, headers, CORS, TLS 1.3, serialization, error boundaries, request deduplication, optimistic concurrency (ETags) | `01_WEB_REQUEST_LIFECYCLE` & `06_APIS` | Multiple API integrations across `axis_clean`, `science-of-our-world` | 🛡️ INTERVIEW READY (Exit Test Passed) | Frontend consumption, contract enforcement, and resilient error recovery; do not claim backend microservices authorship. |
| **8** | **Intelligent UX (Async State & Caching)** | Debouncing, throttling, race conditions, stale closures, query keys, stale-while-revalidate, TanStack Query | `02_JAVASCRIPT_BEFORE_REACT` & `06_APIS` | Async search and data fetching in `axis_clean`; verified Q11.5 (Debouncing vs AbortController vs Request Identity) | 🛡️ INTERVIEW READY (Q11.5 Verified) | Articulate query-key identity and stale-result protection; do not claim TanStack Query makes application-level authorization unnecessary. |
...
| **10** | **Scalability & Performance** | Code splitting (`React.lazy`), bundle analysis, virtualization for large lists, memory leak profiling, layout thrashing prevention | `08_PERFORMANCE` & `03_REACT` | High-volume canvas simulations in `science-of-our-world`; verified Q10.1 (DOM virtualization math, dynamic row heights, 4 layers of data scalability) | 🛡️ INTERVIEW READY (Q10.1 Verified) | Understand DOM virtualization vs JS heap vs network bounds; do not claim building a custom virtualization library from scratch. |
...
| **13** | **Automated Testing Discipline** | Unit testing, React Testing Library, User Event, Mock Service Worker (MSW), integration tests, E2E (Playwright) | `07_TESTING` | Vitest and RTL in `axis_clean/apps/web`; verified production answers for Q11.1, Q11.3 (MSW vs fetch), Q11.4 (Nondeterministic AI streams) | 🛡️ INTERVIEW READY (Q11.1, Q11.3, Q11.4 Verified) | Component behavioral integration + pure reducer state machine testing; do not claim QA automation architect title. |
